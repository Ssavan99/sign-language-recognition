...
- Current GitHub documentation says a public-to-private change detaches any public forks rather than making them private and erases stars and watchers. The zero-fork result removes the fork-owner consequence, but the two watcher subscriptions would be erased. GitHub documents private personal repositories as accessible to explicitly shared collaborators and does not list collaborator removal as a visibility-change consequence; collaborator access is therefore expected to remain and would be verified without modification after any approved change. GitHub does not provide a no-notification guarantee, so the visibility change cannot be characterized as certainly unnoticeable.
...
- The evidence and both options were presented for explicit approval. The former repository remains unchanged while the decision is pending, and its visibility must not be changed before the new pull request is merged and approval is given.
