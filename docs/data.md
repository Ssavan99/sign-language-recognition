...
The maintained CLI accepts an explicit data root and validates discovered class directories before it prepares manifests or trains a model. The historical `Datasets/` directory remains ignored and may be used as a local migration source, but it is not the maintained layout.
...
The official `kagglehub` client can download ordinary public datasets without authentication. The CLI stops with a clear message and accepts a user-supplied local directory if a source later requires consent or authentication; it never prompts for an account or token.
